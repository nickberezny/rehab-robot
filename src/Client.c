...
void * clientThread (void * d)
{
    ...
    while(!quitThreads)
    {
        ...
        if(iter_client == 3)
        {
            ...
            {
                sprintf(buffer, "PLOT::%.4f::%.4f::%.4f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::", 
                    s_client->x/controlParams->xend, s_client->xv/controlParams->xend,
                    s_client->x0_to_send,s_client->Fext,s_client->Text,s_client->cmd,
                    s_client->emg1,s_client->emg2,s_client->emg3,s_client->emg4,
                    s_client->qhip,s_client->qknee,s_client->kest[0],s_client->kest[1]);
            }
            else
            {
                sprintf(buffer, "PLOT::%.4f::%.4f::%.4f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::%.2f::", 
                    s_client->x/controlParams->xend, s_client->xv/controlParams->xend,
                    s_client->x0_to_send/controlParams->xend,s_client->Fext,s_client->Text,s_client->cmd,
                    s_client->emg1,s_client->emg2,s_client->emg3,s_client->emg4,
                    s_client->qhip,s_client->qknee,s_client->kest[0],s_client->kest[1]);
